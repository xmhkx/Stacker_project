//  i2c_LCD.h
//Has added command to turn on/off the backlight.
/*
 * Modified Hi-Tech C I2C library for 16F1829
...
//void I2C_LCD_BLoff(char)   note put 0=off and 1=on
//Only call this function outside of other I2C_LCD functions
//all other functions turn the display back on because that bit is included as 1
void I2C_LCD_BLoff(unsigned char Slave, char Disp){
    
    i2c_Start();      					// send Start
	i2c_Address(Slave, I2C_WRITE);	// Send  slave address with write operation 
    if(Disp) i2c_Write(0x08);
    else 	 i2c_Write(0x00);
 	i2c_Stop();	
